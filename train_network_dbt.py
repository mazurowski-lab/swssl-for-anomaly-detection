...
# BT Loss
def twin_loss(f_patch1, f_patch2, f_neg=None, target=None):
...
def train(model, device, args):
    ...
    # Optimizer
    # DBT
    optimizer = torch.optim.SGD(model.parameters(), lr=5e-6, momentum=0.9, weight_decay=1e-5)
    # Chest
    #optimizer = torch.optim.SGD(model.parameters(), lr=1e-4, momentum=0.9, weight_decay=1e-5)

    # Real Training + Eval Loop
